...
```{r objName-ex02, echo = T, eval = T, prompt = F}
3x <- 7
```

```{r objName-ex03, echo = T, eval = T, prompt = F}
_x <- c("M", "M", "F")
```

```{r objName-ex04, echo = T, eval = T, prompt = F}
.3 <- 10
```
...
> **참고 2: 수치 연산자는 스칼라 뿐 아니라 아래에서 다룰 벡터, 행렬, 리스트, 데이터프레임 객체의 연산에 사용 가능하다.**
...
> 참고: 자료에 `NA`가 포함된 경우 연산 결과는 모두 `NA`로 변환
...
- 결측(`NA`) 처리 함수: `NA` 값에 따라 처리를 다르게 하기 위한 함수
    ...
    - 보통 R의 선형모형과 관련된 함수에서 사용되며, 이들 함수의 `na.action`에 대한 인자값으로 사용됨
...
- 데이터프레임 요약 통계량은 `summary()` 함수를 통해 확인 가능(모든 객체에 적용 가능)
    - 최솟값(minimum), 1사분위수(1$^{\mathrm{st}}$ quantile), 중앙값(median), 평균(mean), 3사분위수(3$^{\mathrm{rd}}$ quantile), 최댓값(maximum) 순으로 각 변수 요약
...
```{r echo = T, eval = T, prompt = F, out.width="\\textwidth"}
...
## 데이터 처리 예시를 위한 실습 자료 설명

### Iris dataset

- 영국 통계학자 Ronald Aylmer Fisher가 소개한 데이터로 붓꽃의 세 가지 종(setosa, versicolor, virginica)에 대해 꽃받침(sepal)과 꽃입(petal)의 길이(length)와 너비(width)를 정리한 자료
...
kable(tab3, booktabs = T, escape = F, caption = "Iris 자료 설명") %>% 
  kable_styling(latex_options = c("striped", "HOLD_position"), font_size = 9)
```

- 붓꽃 별 50행 씩 총 `r NROW(iris)`행으로 이루어짐. 
...
### Diastolic blood pressure dataset

- 책 "Clinical Trial Data Analysis Using R" [@Chen-2010]의 예시자료
- 1960년대 후반 혈압강하제의 유효성 평가를 위한 실시한 RCT 결과 자료
    - 신약(혈압강하제) vs. 위약(placebo)군 비교
    - 당시 이완기 혈압의 고혈압 기준은 $\mathrm{DBP} > 95$ mmHg
...
```{r DBP-desc, echo = F}
library(readxl)
DBP <- readxl::read_xlsx("output/DBP.xlsx")
변수명 <- names(DBP)
설명 <- c("대상자 ID", "처리: 신약(A), 위약(B)", "Baseline", "투약 후 1개월", "투약 후 2개월", "투약 후 3개월", "투약 후 4개월", 
        "나이", "성별: 남성(M), 여성(F)")
타입 <- c("numeric", "character", "numeric", "numeric", "numeric", "numeric", "numeric", "numeric", "character")
tab4 <- data.frame(변수명, 설명, 타입)
kable(tab4, booktabs = T, escape = F, caption = "DBP 자료 설명") %>% 
  kable_styling(latex_options = c("striped", "HOLD_position"), font_size = 9)
```
...
### RStudio에서 외부 파일 읽어보기

- 최근 Rstudio version 1.1.xxx 이후 손쉽게 외부 자료를 R workspace 에 불러오는 것이 가능해짐. 
- Rstudio 상단 메뉴에서
    - `[File]` $\rightarrow$ `[Import Dataset]`에서 조건에 맞는 파일 옵션 선택
...
{
  \centering
  \includegraphics[width = 15cm, height = 10cm]{Figures/data-import.png}
  \caption[]{R-Studio 파일 메뉴에서 외부파일 읽어오기(예시)}\label{fig:data-import}
}
\end{figure}

### R 명령어를 이용한 외부 파일 읽어오기/내보내기
...
- `read.table()` 함수의 기본 사용 형태

```{r echo = T, eval = F}
read.table(file = "", # 읽어올 *.txt 또는 *.csv 파일 이름 
           sep = "", # 자료 구분 분리자(e.g. TAB, comma, space, ...)
           header = TRUE, # 파일 첫 행을 헤더(변수명)로 처리할 것인지 여부 
           na.strings = "NA", # "NA"로 저장된 문자열은 R의 결측으로 처리
           stringsAsFactors = TRUE, # 문자열 벡터의 요인형 변환 여부
           )
```

- 예시: 하위폴더 `output`에 저장되어 있는 `DBP.txt`를 `read.table()` 함수를 이용해 불러오기

```{r echo = T, eval = T, prompt = T}
DBP <- read.table("output/DBP.txt", sep = "\t", header = T)
head(DBP)
```
