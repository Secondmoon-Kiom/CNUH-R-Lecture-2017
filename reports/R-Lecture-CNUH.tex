...
\colorbox{gray!10}{\begin{minipage}{15cm}
\textbf{Tips}
\texttt{vignette}에서 제공하는 문서는 실제 분석을 기반으로 작성한 문서이기 때문에 초보자들이 R 패키지의 접근성을 높혀줌. \texttt{browseVignettes()}으로 현존하는 모든 R 패키지들의 vignette을 볼 수 있기 때문에 매우 유용함.
\end{minipage}}
\end{quote}
...
\item
  ...
  \item
    ...
    \end{itemize}
  \item
    Help: \texttt{help(명령어)} 입력 시 도움말 창이 출력되는 공간
  \end{enumerate}
\end{enumerate}
...
\section{rJava 설치하기}\label{rjava-}
...
\section{RStudio를 잘 활용 하려면?}\label{rstudio---}
...
\item
  ...
  것이 R의 특징임
\item
  들어가기에 앞서 R의 데이터 타입에 대한 개괄적인 모식도를 먼저 확인
\end{itemize}

\begin{figure}[H] {
  \centering
  \includegraphics[width = 15cm, height = 13cm]{Figures/datatype-diagram}
  \caption[R 데이터 타입 구조 다이어그램]{R 데이터 타입 구조 다이어그램}\label{fig:Rstudio-hlpwin}
} \end{figure}
...
\begin{quote}
\colorbox{gray!10}{\begin{minipage}{15cm}
\textbf{Tips: R을 본격적으로 다루기 앞서 알아두면 유용한 단축키 및 명령어}
  \begin{itemize}
